...
class JsRuntime
{
     ...
     * @param string    $js_str     JavaScript code or format string of JavaScript code
     * @param array     $sp_value   values for format specifiers
     ...
     */
    public function run(string $type, string $js_str, array $sp_value = [], string $value = null): ?string
    {
        if (empty(static::$app))
        ...
        $result = null;
        try {
            $js_code = empty($sp_value) ? $js_str : vsprintf($js_str, $sp_value);
        } catch (\Throwable $e) {
        ...
        if (static::$ver == '(remote)') {
            ...
            $tmp_file = $this->getTempDir() . 'yt_' . hash('sha1', uniqid('', true)) . '.dump';
            if (file_put_contents($tmp_file, $js_code) === false) {
                throw new YouTubeException('Failed to create files in ' . $this->getTempDir());

            } else {
                $result = exec(static::$app . ' ' . static::$arg . " {$tmp_file}", $output, $result_code);
                unlink($tmp_file);
            }
            ...
            if (empty($result) || ($result == $value)) {
                ...
                if (!empty($result_code)) {
                    throw new YouTubeException("Exit status {$result_code} ('{$jsr_exe} " . static::$ver . "')");
                } else {
                    throw new YouTubeException(
                        "Failed to solve {$type} ("
                        . (!empty($sp_value) ? "func:'" . substr($sp_value[1], 0, 20) . "' " : '')
                        . "'{$jsr_exe} " . static::$ver . "')");
                }
            }
        }
