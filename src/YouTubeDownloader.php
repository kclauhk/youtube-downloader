...
class YouTubeDownloader
{
    ...
    protected function getPlayerApiResponse(string $video_id, string $clientName, YouTubeConfigData $configData): PlayerApiResponse
    {
    ...
    public function getDownloadLinks(string $video_id, $clients = ['android_testsuite'], array $extra = []): DownloadOptions
    {
        ...
        $links = [];
        $clients = is_array($clients) ? $clients : explode(',', preg_replace('/\s+/', '', $clients));
        foreach($clients as $clientName) {
            ...
            $player_response = $this->getPlayerApiResponse($video_id, strtolower($clientName), $youtube_config_data);
