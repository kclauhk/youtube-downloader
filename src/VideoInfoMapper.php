            ...
            foreach ($cmdRuns as $v) {
                if ($c = Utils::arrayGet($v, 'onTap.innertubeCommand')) {
                    if ($url = Utils::arrayGet($c, 'browseEndpoint.canonicalBaseUrl')) {
                        ...
                        if (strpos($url, '://www.youtube.com/redirect?')) {
                            parse_str($url, $qs);
                            $url = $qs['q'];
                        }
                        ...
                        } elseif ($path = Utils::arrayGet($c, 'commandMetadata.webCommandMetadata.url')) {
                    ...
                    if (!empty($url)) {
                        if (
                            $url[-1] == '/'
                            && strpos($referenceDesc, $url) === false
                            && strpos($referenceDesc, substr($url, 0, strlen($url) - 1)) !== false
                        ) {
